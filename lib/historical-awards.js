...
const ENVIRONMENTAL_SCOPE_PATTERN = /\b(environmental site (?:assessment|investigation)|contaminat(?:ed|ion))\b/i;
const CORE_GEOTECH_PATTERN = /\b(geotechnical|geotech|ground investigation|soil investigation|sub[ -]?surface investigation|bore[ -]?holes?|rock cor(?:e|ing)|cptu?|dynamic cone penetrometer|pavement investigation|piezometers?)\b/i;
...
const OPENCONTRACTAU_JURISDICTIONS = Object.freeze({
  ...
  SCENIC_RIM: "QLD",
  SYDNEY_COUNCIL: "NSW",
  NORTHERN_BEACHES: "NSW",
  WOLLONGONG: "NSW",
  BLACKTOWN: "NSW",
  CUMBERLAND: "NSW",
  LIVERPOOL_NSW: "NSW",
  CENTRAL_COAST_NSW: "NSW",
  SUTHERLAND_SHIRE: "NSW",
  STIRLING_WA: "WA",
  MELBOURNE: "VIC",
  MORNINGTON_PENINSULA: "VIC",
  WYNDHAM: "VIC",
  BOROONDARA: "VIC",
  GEELONG: "VIC",
  HOBSONS_BAY: "VIC",
...
export function classifyGeotechnical(record, threshold = 25) {
  ...
  if (ENVIRONMENTAL_SCOPE_PATTERN.test(fullText) && !CORE_GEOTECH_PATTERN.test(fullText)) score = Math.min(score, threshold - 1);
...
export function normalizeOpenContractAuRelease(
  release,
  packageMetadata = {},
  localSource = null,
  threshold = 25,
) {
  if (!release || !Array.isArray(release.awards)) return [];
  const jurisdictionKey = cleanText(
    release.source?._jurisdiction ?? packageMetadata.jurisdiction ?? "UNKNOWN",
  )?.toUpperCase() ?? "UNKNOWN";
  const jurisdiction = OPENCONTRACTAU_JURISDICTIONS[jurisdictionKey] ?? jurisdictionKey;
  const tender = release.tender ?? {};
  const contractsByAward = new Map(
    (release.contracts ?? []).map((contract) => [contract.awardID, contract]),
  );
  const sourceUrl = cleanText(
    release.source?.url ?? release.source?.uri ?? packageMetadata.uri,
  );
  ...
  return release.awards.flatMap((award) => {
    ...
    const itemDescriptions = compact([
      ...(tender.items ?? []).map((item) => item.description),
      ...(award.items ?? []).map((item) => item.description),
      ...(contract.items ?? []).map((item) => item.description),
    ]);
    const categories = compact([
      ...(tender.items ?? []).map((item) => item.classification?.description ?? item.classification?.id),
      ...(award.items ?? []).map((item) => item.classification?.description ?? item.classification?.id),
      ...(contract.items ?? []).map((item) => item.classification?.description ?? item.classification?.id),
      release.source?.category,
    ]);
    const locations = compact([
      ...(tender.items ?? []).map((item) => item.deliveryLocation?.description),
      ...(award.items ?? []).map((item) => item.deliveryLocation?.description),
      ...(contract.items ?? []).map((item) => item.deliveryLocation?.description),
      release.source?.location,
    ]);
    const period = contract.period ?? award.contractPeriod ?? tender.contractPeriod ?? {};

    return suppliers.map((supplier) => buildRecord({
      tenderId: tender.id ?? release.ocid,
      contractId: contract.id ?? award.id,
      title: contract.title ?? award.title ?? tender.title,
      scope: compact([
        tender.description,
        award.description,
      ...
      agency: release.buyer?.name,
      supplierName: supplier.name ?? supplier.identifier?.legalName,
      supplierABN: supplier.identifier?.scheme?.toUpperCase() === "AU-ABN"
        ? supplier.identifier.id
        : null,
      awardValue: contract.value?.amount ?? award.value?.amount ?? tender.value?.amount,
      currency: contract.value?.currency ?? award.value?.currency ?? tender.value?.currency,
      publishDate: release.date,
      awardDate: award.date ?? contract.dateSigned,
      ...
      tenderUrl: sourceUrl,
      awardUrl: sourceUrl,
      sourcePortal: `OpenContractAU ${jurisdictionKey}`,
      sourceUrl,
      sourceRecordId: contract.id ?? award.id ?? release.id ?? release.ocid,
      tenderMatchStatus: tender.id ? "ocds-release-match" : "award-only",
      scopeSource: "OpenContractAU OCDS tender, award, contract and item descriptions",
      collectedAt: packageMetadata.collectedAt,
      provenance: {
        sourceSystem: "OpenContractAU",
        upstreamRepository: "https://github.com/demitonapp/opencontractau",
        upstreamCommit: packageMetadata.upstreamCommit ?? null,
        upstreamPublisher: packageMetadata.publisher?.uid ?? packageMetadata.publisher?.name ?? null,
        upstreamLicense: packageMetadata.license ?? null,
        packageUri: packageMetadata.uri ?? null,
        localSource,
        jurisdictionKey,
        ocid: release.ocid ?? null,
        releaseId: release.id ?? null,
        originalSource: release.source ?? {},
        extractionMethod: "opencontractau-ocds-package-import",
      },
    }, threshold));
  });
}
