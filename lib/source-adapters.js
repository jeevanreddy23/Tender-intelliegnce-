...
function isoDate(value) {
  ...
  const namedMonthDate = text.match(/^(\d{1,2})[\/\-\s]([A-Za-z]{3})[\/\-\s](\d{4})(?:\s+(\d{1,2}):(\d{2})(?:\s*([ap]m))?)?(?:\s*\(\s*(?:UTC|GMT)\s*([+-])(\d{1,2})(?::?(\d{2}))?\s*\))?/i);
  const slashDateTime = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:\s+(\d{1,2}):(\d{2})(?:\s*([ap]m))?)?(?:\s*\(\s*(?:UTC|GMT)\s*([+-])(\d{1,2})(?::?(\d{2}))?\s*\))?/i);
  ...
  if (timedMatch && (namedMonthDate ? monthIndex >= 0 : true)) {
    ...
    const utcOffsetMinutes = timedMatch[7]
      ? (timedMatch[7] === "-" ? -1 : 1) * (Number(timedMatch[8]) * 60 + Number(timedMatch[9] ?? 0))
      : 0;
