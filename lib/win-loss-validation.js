...
export const WIN_DRIVER_DEFINITIONS = Object.freeze([
  Object.freeze({
    ...
    reviewAction: "Attach dated winner capability evidence and the buyer's attributable selection rationale.",
    statement(winner) {
  ...
  Object.freeze({
    ...
    reviewAction: "Attach the evaluated winning bid, a comparable conforming bid, and the buyer's selection rationale.",
    statement(winner) {
  ...
  Object.freeze({
    ...
    reviewAction: "Attach attributable prior-relationship evidence and confirmation that the buyer evaluated it.",
    statement(winner) {
  ...
  Object.freeze({
    ...
    reviewAction: "Attach the tender timeline, the winner's evaluated commitment, and the buyer's selection rationale.",
    statement(winner) {
...
export function buildWinLossPremise(record, evidenceItems = [], options = {}) {
  ...
  const evidenceResults = suppliedEvidence.slice(0, MAX_EVIDENCE_ITEMS).map((item, index) => {
    ...
    if (result.valid && DATED_CAPABILITY_KINDS.has(result.evidence.kind)) {
      if (recordPeriod === null) {
        return { valid: false, reason: "capability evidence requires a valid relevant tender or award date", index };
      }
...
export function evaluateWinHypothesis(premisePackage, hypothesis, nliResult, options = {}) {
  ...
  const nliAttempted = missingEvidence.length === 0 && nliResult !== undefined && nliResult !== null;
  ...
  return {
    ...
    nli: normalized,
    nliAttempted,
    nliCompleted: nliAttempted && Boolean(normalized),
    nliFailed: nliAttempted && !normalized,
...
function modelRunCounts(evaluations) {
  return {
    modelCallsAttempted: evaluations.filter(({ nliAttempted }) => nliAttempted).length,
    modelCallsCompleted: evaluations.filter(({ nliCompleted }) => nliCompleted).length,
    modelCallsFailed: evaluations.filter(({ nliFailed }) => nliFailed).length,
  };
}

function autopsyStatus(evaluations) {
  const counts = modelRunCounts(evaluations);
  if (!counts.modelCallsAttempted) return "BLOCKED_BY_EVIDENCE";
  if (!counts.modelCallsCompleted) return "MODEL_FAILURE";
  return counts.modelCallsCompleted === evaluations.length && !counts.modelCallsFailed ? "COMPLETE" : "PARTIAL";
}

/**
 * Sanitized historical award assessment for an authenticated detail surface.
 * It intentionally excludes the full premise and source text and can never
 * mutate a current opportunity score or establish a causal award explanation.
 */
export function buildWinLossRecordAutopsy(record, options = {}) {
  const result = evaluateWinLossRecord(record, options);
  const hypotheses = result.evaluations.map((evaluation) => ({
    ...
    readyForNli: evaluation.readyForNli,
    missingEvidence: evaluation.missingEvidence,
    evidence: evaluation.evidence,
    nliAttempted: evaluation.nliAttempted,
    nliCompleted: evaluation.nliCompleted,
    nliFailed: evaluation.nliFailed,
  ...
  return {
    ...
    status: autopsyStatus(hypotheses),
    hypotheses,
    verdictCounts: countVerdicts(hypotheses),
    ...modelRunCounts(hypotheses),
    ...
    interpretation: "NLI tests semantic support in attributable historical evidence; it does not prove why an award was made.",
  };
}
...
export function buildWinLossValidationSummary(records, options = {}) {
  const recordAutopsies = records.map((record) => {
    const identity = recordId(record);
    return buildWinLossRecordAutopsy(record, {
      ...options,
      evidence: optionForRecord(options.evidenceByRecord, identity) ?? [],
      nliResults: optionForRecord(options.nliResultsByRecord, identity) ?? {},
    });
  });
  const evaluations = recordAutopsies.flatMap(({ hypotheses }) => hypotheses);
  ...
  const runCounts = modelRunCounts(evaluations);
  const nliEvaluationsRun = runCounts.modelCallsCompleted;
  ...
  const driverAutopsy = {
    status: runCounts.modelCallsAttempted
      ? runCounts.modelCallsCompleted ? "REVIEW_AVAILABLE" : "MODEL_FAILURE"
      : "BLOCKED_BY_EVIDENCE",
    supportedHistoricalFindings: verdictCounts.supported,
    summary: runCounts.modelCallsAttempted
      ? `${runCounts.modelCallsCompleted} model calls completed; review supported and contradicted findings with their evidence.`
      : "No historical hypothesis is ready for semantic validation because attributable evaluation evidence is missing.",
    drivers: WIN_DRIVER_DEFINITIONS.map(({ id, label, reviewAction }) => {
      const counts = byDriver[id];
      const state = counts.supported && counts.contradicted
        ? "MIXED_FINDINGS"
        : counts.supported
          ? "SUPPORTED_FINDINGS"
          : counts.contradicted
            ...
            : counts.readyForNli
              ? "READY_FOR_NLI"
              : "EVIDENCE_REQUIRED";
      const statusLabel = state === "MIXED_FINDINGS"
        ? "Mixed historical findings"
        : state === "SUPPORTED_FINDINGS"
          ? "Supported historical evidence"
          : state === "CONTRADICTED_FINDINGS"
            ? "Contradicted historical evidence"
            : state === "READY_FOR_NLI"
              ? "Ready for NLI"
              : "Evidence required";
      return { id, label, state, statusLabel, ...counts, reviewAction };
    }),
    causalClaimAllowed: false,
    scoreMutationAllowed: false,
    humanReviewRequired: true,
  ...
  return {
    ...
    status: runCounts.modelCallsAttempted
      ? runCounts.modelCallsCompleted ? "PARTIAL_OR_COMPLETE" : "MODEL_FAILURE"
      : "BLOCKED_BY_EVIDENCE",
    ...
    reason: runCounts.modelCallsAttempted
      ? runCounts.modelCallsCompleted
        ? "Only hypotheses with complete attributable evidence were sent to NLI."
        : "NLI was attempted for evidence-ready hypotheses, but every attempted model result failed validation."
