...
test("keeps starter-only code out of the product surface", async () => {
  ...
  assert.match(layout, /<html lang="en" className=\{`\$\{geistSans\.variable\} \$\{geistMono\.variable\}`\}>/);
  assert.match(css, /--background:#f4f1e9/);
  assert.match(css, /--surface:#fffefa/);
  assert.match(css, /--ink:#15231f/);
  assert.match(css, /--forest:#1c5947/);
  assert.match(css, /--orange:#db6b3b/);
  assert.match(css, /--orange-ink:#a34829/);
  assert.match(css, /--font-mono:var\(--font-geist-mono\)/);
  assert.doesNotMatch(css, /--background:#eaf0ec/);
  assert.match(css, /--forest:/);
  assert.match(css, /@media\(max-width:860px\)/);
  assert.match(css, /@media\(max-width:560px\)/);
  assert.match(css, /\.opportunity-row\{grid-template-columns:47px minmax\(0,1fr\)\}/);
  assert.match(css, /\.strategy-memo\{grid-template-columns:1fr\}/);
