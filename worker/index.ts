...
const visibleOpportunityPredicate = `
  (
    (r.closing_at IS NOT NULL AND TRIM(r.closing_at) <> '' AND datetime(r.closing_at) >= datetime('now'))
    OR ((r.closing_at IS NULL OR TRIM(r.closing_at) = '') AND datetime(r.updated_at) >= datetime('now', '-7 days'))
  )
  AND LOWER(TRIM(COALESCE(json_extract(r.authoritative_json, '$.status'), ''))) NOT LIKE '%closed%'
  AND LOWER(TRIM(COALESCE(json_extract(r.authoritative_json, '$.status'), ''))) NOT LIKE '%expired%'
  AND LOWER(TRIM(COALESCE(json_extract(r.authoritative_json, '$.status'), ''))) NOT LIKE '%cancel%'
  AND LOWER(TRIM(COALESCE(json_extract(r.authoritative_json, '$.status'), ''))) NOT LIKE '%award%'
  AND LOWER(TRIM(COALESCE(json_extract(r.authoritative_json, '$.status'), ''))) NOT LIKE '%complet%'
  AND LOWER(TRIM(COALESCE(json_extract(r.authoritative_json, '$.status'), ''))) NOT LIKE '%withdraw%'
